...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	function reportError(test, error, indent, prefix, parentPath) {
		...
		if (typeof schemaValue !== 'undefined') {
			out.writeln(prefix + indent + schemaValue + style.accent(' -> ') + tweakPath(error.schemaPath));
		}
		else {
	...
	function reportBulk(failed, passed, indent) {
		...
		if (failed.length > 0) {
			...
			out.writeln('');
			out.writeln(style.error('>> ') + 'tv4 ' + (passed.length > 0 ? style.warning('validated ' + passed.length) + ', ' : '') + style.error('failed ' + failed.length) + ' of ' + style.error(total + ' ' + pluralise('value', total)));
		}
		else if (total === 0) {
			//out.writeln('');
			...
			//out.writeln('');
			out.writeln(style.success('>> ') + 'tv4 ' + style.success('validated ' + passed.length) + ' of ' + style.success(total + ' ' + pluralise('value', total)));
		}
