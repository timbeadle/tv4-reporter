...
const getReporter = (out, style) => {
	...
	const tweakMessage = (str) => {
		return style.warning(str.charAt(0).toLowerCase() + str.slice(1));
	};
	...
	function reportTotals(numFailed, numPassed) {
		const total = numFailed + numPassed;
		if (numFailed > 0) {
			out.writeln(style.error('>> ') + 'tv4 ' + (numPassed > 0 ? style.warning('validated ' + numPassed) + ', ' : '') + style.error('failed ' + numFailed) + ' of ' + style.error(total + ' ' + utils.pluralise('value', total)));
		} else if (total === 0) {
			// (was) out.writeln('');
			out.writeln(style.warning('>> ') + 'tv4 ' + style.warning('validated zero values'));
		} else {
			// (was) out.writeln('');
			out.writeln(style.success('>> ') + 'tv4 ' + style.success('validated ' + numPassed) + ' of ' + style.success(total + ' ' + utils.pluralise('value', total)));
		}
	}
